const { Function: Func, Logs, Scraper, Cooldown, Spam, InvCloud } = new(require('@neoxr/neoxr-js'))
...
module.exports = async (client, ctx) => {
   ...
   client.cooldown = client.cooldown ? client.cooldown : []
   if (m && m.message) {
   ...
   try {
      ...
      if (body && prefix && commands.includes(command) || body && !prefix && commands.includes(command) && setting.noprefix || body && !prefix && commands.includes(command) && env.evaluate_chars.includes(command)) {
         ...
         for (let name in is_commands) {
            ...
            if (!m.isGroup && !['owner', 'menfess', 'jadibot', 'verify', 'payment', 'premium', 'buyprem', 'sewa'].includes(name) && chats && !isPrem && !users.banned && new Date() * 1 - chats.lastchat < env.timeout) continue
            if (!m.isGroup && !['owner', 'menfess', 'jadibot', 'verify', 'payment', 'premium', 'buyprem', 'sewa'].includes(name) && chats && !isPrem && !isOwner && !users.banned && setting.groupmode) {
         ...
         for (let name in is_events) {
            ...
            if (!m.isGroup && !['menfess_ev', 'chatbot'].includes(name) && chats && !isPrem && !users.banned && new Date() * 1 - chats.lastchat < env.timeout) continue
            if (!m.isGroup && setting.groupmode && (!global.db.menfess.some(v => v.from === m.sender) && !global.db.menfess.some(v => v.receiver === m.sender)) && !['system_ev', 'menfess_ev', 'chatbot', 'auto_download'].includes(name) && !isPrem && !isOwner) return client.sendMessageModify(m.chat, `⚠️ Using bot in private chat only for premium user, want to upgrade to premium plan ? send *${prefixes[0]}premium* to see benefit and prices.`, m, {
               largeThumb: true,
            ...
            if (!event.game && event.limit && users.limit < 1 && body && Func.generateLink(body) && Func.generateLink(body).some(v => Func.socmed(v))) return client.reply(m.chat, `⚠️ You reached the limit and will be reset at 00.00\n\nTo get more limits upgrade to premium plan.`, m).then(() => {
               users.premium = false
               users.expired = 0
            })
            ...
            if (event.download && users && !users.verified && body && Func.generateLink(body) && Func.generateLink(body).some(v => Func.socmed(v)) && setting.verify) return client.reply(m.chat, `⚠️ Nomor kamu belum terverifikasi, untuk melakukan verifikasi gunakan perintah *${prefixes[0]}reg email*\n\n*Contoh* : ${prefixes[0]}reg neoxrbot@gmail.com`, m)
            ...
            if (event.download && body && !isPrem && setting.autodownload && Func.generateLink(body) && Func.generateLink(body).some(v => Func.socmed(v))) return client.reply(m.chat, `⚠️ Auto download is only for premium users, use command to download️.`, m)
